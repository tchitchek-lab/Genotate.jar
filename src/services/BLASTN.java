 ...
 * BLASTN
 */
public class BLASTN extends SERVICE{
	...
	public String run(File computing_file,File result_file,File fastatempnucl,File fastatempnc,File fastatempprot,List<String> options,Hashtable<String, String> path_services) throws Exception{
		...
		if(path_services.containsKey("BLAST")){
			path = path_services.get("BLAST")+"/blastn";
		}else{
		...
		String cmd = path+" -num_threads "+threads+" -query "+fastatempnucl.getAbsolutePath()+output+computing_file.getAbsolutePath()+" -db "+dir_path_database+"/"+db;
	...
	public  ArrayList<Annotation>  parse(File result_file) throws Exception{
		...
		while((line=reader.readLine())!=null){
			try {
				...
				int pos_begin = Integer.parseInt(splittab[idx_posbegin])-1;
				int pos_end = Integer.parseInt(splittab[idx_posend])-1;
				int length = Integer.parseInt(splittab[idx_length]);
