 ...
 * MHCI
 */
public class MHCI extends SERVICE  {
	...
	public String run(File computing_file,File result_file,File fastatempnucl,File fastatempnc,File fastatempprot,List<String> options,Hashtable<String, String> path_services) throws Exception{
		if(path_services.containsKey("MHCI")){
			path = path_services.get("MHCI");
		}else{
			throw new Exception("Path to MHCI not configured");
		}
		if(options.size()>0){
		...
		reader.close();
		String command = path + " IEDB_recommended HLA-A*02:01 9 "+fastatempprot.getAbsolutePath()+" >> "+computing_file.getAbsolutePath();
	...
	public ArrayList<Annotation> parse(File result_file) throws Exception{
		...
		String service = "MHCI";
		String name = "MHC-I binding site";
		int tmp_region_id = -1;
		int tmp_begin = -1;
		int tmp_end = -1;
		String line;
